...
module.exports = async () => {
  ...
  if (command === "setup") {
    ...
    childProcess.execSync("forge init . --no-deps --force", {
      stdio: [0, 1, 2],
    });
    ...
    childProcess.execSync(
      "forge install foundry-rs/forge-std@v1.5.6 OpenZeppelin/openzeppelin-contracts@v4.9.1",
      { stdio: [0, 1, 2] }
    );

    // go ahead and test to build up the cache/compilation artifacts
    childProcess.execSync("forge test -vv --json", { stdio: [0, 1, 2] });
  } else if (command === "run") {
    await copyContents();
